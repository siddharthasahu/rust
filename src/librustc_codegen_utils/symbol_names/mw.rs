extern crate std_mangle_rs as mw_mangle;
use self::mw_mangle::{ast, compress::compress_ext};
...
pub(super) fn mangle(
    ...
    let instantiating_crate = super::instantiating_crate(tcx, instance);
...
impl Printer<'tcx, 'tcx> for SymbolPrinter<'_, 'tcx> {
    ...
    fn print_type(
        ...
        Ok(Arc::new(match ty.sty {
            ...
            ty::Array(ty, len) => {
                let len = match len {
                    ty::LazyConst::Unevaluated(..) => return Err(Unsupported),
                    ty::LazyConst::Evaluated(c) => c.unwrap_usize(self.tcx),
                };
    ...
    fn path_crate(
        ...
        let path = ast::PathPrefix::CrateId {
            ...
            dis: self.tcx.crate_disambiguator(cnum).to_string(),
        };
    ...
    fn path_qualified(
        ...
        // HACK(eddyb) this uses disambiguator 0 while actual impls have
        // their disambiguator incremented by 1 (see `path_append_impl`).
    ...
    fn path_append_impl(
        ...
        let path = ast::PathPrefix::TraitImpl {
            ...
            // See `path_qualified` above for why `1 +`.
            dis: ast::NumericDisambiguator(1 + disambiguated_data.disambiguator as u64),
        };
        Ok(Arc::new(ast::AbsolutePath::Path {
            name: Arc::new(path),
            args: ast::GenericArgumentList::new_empty(),
    ...
    fn path_append(
        ...
        let tag = match disambiguated_data.data {
            DefPathData::StructCtor => {
                let ident = match Arc::make_mut(prefix) {
                    ast::PathPrefix::Node { ident, .. } => ident,
                    _ => unreachable!(),
                };
                ident.tag = ast::IdentTag::ValueNs;
                return Ok(path);
            }
            ...
            _ => ast::IdentTag::TypeNs,
        };

        let ident = match tag {
            ast::IdentTag::Closure => String::new(),
